...
*/
        ...
        //cv::imwrite("RGB_Image.jpg", rgbImage);
         ...
         for (auto& point: *cloudo)
      {
        ...
        if (u >= 0 && u < rgbImage.cols && v >= 0 && v < rgbImage.rows) {
            ...
            if(depth<1){
             RCLCPP_INFO(this->get_logger(), "Publishing: '%lf'", depth);}
            double maxDepth=5;
            double minDepth=0.5;
            ...
            int blue = static_cast<int>(255 * (1 - 100*depth));
            int red = static_cast<int>(255 * 100*depth);

            rgbImage.at<cv::Vec3b>(v, u) = cv::Vec3b(10*depth, 0, 0);
            //cv::circle(rgbImage, cv::Point(u,v),3, cv::Scalar(blue,0,red),-1, 8,0);
        }
        
        
     }
     cv::imwrite("projected_Image.jpg", rgbImage);
